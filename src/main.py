...
from .injestion import store_file_and_register_dataset
from .tools import describe_dataframe, llm_with_tool, load_csv, generate_chart
from .state import AnalysisState
from .registry import DATASET_REGISTRY, update_dataset_access, cleanup_expired_datasets, get_registry_stats, delete_dataset
from .db import conn
...
#             numeric_summary = df.select_dtypes(include='number').describe().loc[['count','mean','min','max']].round(2).to_dict()
            
#             # --- concise categorical summary ---
#             categorical_summary = {}
#             for col in df.select_dtypes(include='object').columns:
#                 top_vals = df[col].value_counts().nlargest(2).to_dict()
...
#                             f"Numeric Summary (count, mean, min, max): {numeric_summary}\n"
#                             f"Categorical Top Values: {categorical_summary}\n"
#                             f"Sample: {df_sample.to_dict(orient='records')}\n"
#                             "You may now analyze it or call generate_chart"
#                         )
#                     )
#                 ]
#             }
